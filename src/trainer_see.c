...
bool8 UpdateBadOnionCounter(void)
{
    ...
    if (steps != 0)
    {
        ...
        if (steps == 0)
        {
            ScriptContext1_SetupScript(EventScript_BwRepelWoreOff); //replace with own script   EventScript_BwTrainerRepelWoreOff
            return TRUE;
        }
    }
    return FALSE;
...
static bool8 CheckTrainer(u8 trainerObjId)
{
    ...
    if (approachDistance != 0)  //0 means trainer won't approach
    {
