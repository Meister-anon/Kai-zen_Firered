...
void InitAndLaunchChosenStatusAnimation(bool8 isStatus2, u32 status)
{
    ...
    if (!isStatus2)
    {
     ...
     //   else if (status == )//most all status arguments are listed in terms of ||, so I need to include some cases for and, so here I will list every possible combo of status
        else // no animation
